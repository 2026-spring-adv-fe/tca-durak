...
export const getGeneralFacts = (games: GameResult[]): GeneralFacts => {
  ...
  return {
    lastPlayed: `${formatLastPlayed(mostRecentlyPlayedInMilliseconds)} ago`,
...
export const getLeaderboard = (games: GameResult[]): LeaderboardEntry[] =>
  getPreviousPlayers(games)
    .map((player) => getLeaderboardEntry(games, player))
    .sort((a, b) =>
...
export const getPreviousPlayers = (games: GameResult[]) =>
  games
    .flatMap((x) => x.players)
    .filter((x, i, a) => i === a.findIndex((y) => y === x))
    .sort((a, b) => a.localeCompare(b));

export const getAvgGameDurationsByPlayerCount = (
  results: GameResult[]
): AvgGameDuration[] => {
  const grouped = Map.groupBy(results, (x) => x.players.length);
...
export const getGameCountByMonth = (
  results: GameResult[]
): gameCountByMonth[] => {
  if (results.length === 0) {
    return [];
  ...
  const grouped = Map.groupBy(
    results,
    (x) =>
      new Date(x.start).toLocaleString("default", {
        month: "short",
      })
  );
...
const getLeaderboardEntry = (
  ...
  const durakTimes = games.filter((x) => x.loser === player).length;

  const totalGames = games.filter((x) =>
    x.players.some((y) => y === player)
  ).length;

  const escapes = totalGames - durakTimes;

  const escapeRate = totalGames > 0 ? escapes / totalGames : 0;
